...
# Рассылка
(BROADCAST_TYPE, BROADCAST_TARGET, BROADCAST_MESSAGE) = range(30, 33)
# Блокировка
BAN_INPUT_TARGET = 40
...
class Database:
    ...
    def block_user(self, user_id):
        conn = self._get_connection()
        conn.execute("INSERT OR IGNORE INTO blocked_users (user_id, blocked_date) VALUES (?, ?)",
    ...
    # ---------- Рейтинг ----------
    def get_top_stars(self, limit=10):
        conn = self._get_connection()
        rows = conn.execute(
...
async def start_registration(update: Update, context):
    ...
    if db.is_blocked(user_id):
        await update.message.reply_text("<b>🚫 Вы заблокированы и не можете использовать бота.</b>", parse_mode='HTML')
...
async def check_sub_callback(update: Update, context):
    ...
    if db.is_blocked(user_id):
        await query.edit_message_text("<b>🚫 Вы заблокированы.</b>", parse_mode='HTML')
        return ConversationHandler.END
...
async def captcha_handler(update: Update, context):
    user_id = update.effective_user.id
    if db.is_blocked(user_id):
        await update.message.reply_text("<b>🚫 Вы заблокированы.</b>", parse_mode='HTML')
        return ConversationHandler.END
...
async def button_handler(update: Update, context):
    ...
    if data == "profile":
        ...
        keyboard = [
            [InlineKeyboardButton("⭐ По звёздам", callback_data="rating_stars")],
            [InlineKeyboardButton("👥 По друзьям", callback_data="rating_refs")],
            [InlineKeyboardButton("🔙 Назад", callback_data="back_to_menu")]
        ]
        await edit_or_reply(query, "<b>🏆 Выберите рейтинг:</b>", InlineKeyboardMarkup(keyboard))

    elif data == "rating_stars":
        top = db.get_top_stars(10)
        if not top:
            ...
            for i, user in enumerate(top, 1):
                ...
                text += f"{i}. {name} ({uname}) — {user['stars']} ⭐\n"
        await edit_or_reply(query, text, InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="rating")]]))

    elif data == "rating_refs":
        top = db.get_top_referrals(10)
        if not top:
            text = "<b>Рейтинг пока пуст.</b>"
        else:
            text = "<b>👥 Топ-10 по приглашённым друзьям:</b>\n\n"
            for i, user in enumerate(top, 1):
                name = html.escape(user['first_name'] or "Без имени")
                uname = f"@{html.escape(user['username'])}" if user['username'] else "—"
                text += f"{i}. {name} ({uname}) — {user['cnt']} друзей\n"
        await edit_or_reply(query, text, InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="rating")]]))
        ...
        # Запускаем соответствующие диалоги
        if data == "admin_broadcast":
            if user_id not in ADMIN_IDS: return
            await query.message.reply_text("<b>📢 Выберите тип рассылки:</b>",
                                           reply_markup=InlineKeyboardMarkup([
                                               [InlineKeyboardButton("Всем", callback_data="broadcast_all"),
                                                InlineKeyboardButton("Одному", callback_data="broadcast_one")],
                                               [InlineKeyboardButton("Отмена", callback_data="cancel_broadcast")]
                                           ]))
            return BROADCAST_TYPE
        elif data == "admin_block":
            if user_id not in ADMIN_IDS: return
            await query.message.reply_text("<b>🚫 Введите @username или ID пользователя, которого нужно заблокировать:</b>",
                                           parse_mode='HTML')
            return BAN_INPUT_TARGET
        elif data == "admin_logs":
            await query.answer("🚧 В разработке")

# ---------- Обработчик сообщений (включая пароль чека и диалоги) ----------
...
# ---------- Диалог рассылки ----------
async def broadcast_type_choice(update: Update, context):
    query = update.callback_query
    await query.answer()
    choice = query.data
    if choice == "cancel_broadcast":
        await query.edit_message_text("<b>❌ Рассылка отменена.</b>", parse_mode='HTML')
        return ConversationHandler.END
    context.user_data['broadcast_target'] = choice  # "broadcast_all" или "broadcast_one"
    if choice == "broadcast_all":
        await query.edit_message_text("<b>📝 Введите сообщение для рассылки всем пользователям:</b>", parse_mode='HTML')
        return BROADCAST_MESSAGE
    else:
        await query.edit_message_text("<b>👤 Введите @username или ID получателя:</b>", parse_mode='HTML')
        return BROADCAST_TARGET

async def broadcast_target_input(update: Update, context):
    ...
    if target_str.startswith('@'):
        ...
        if row:
            ...
            await update.message.reply_text(f"<b>❌ Пользователь @{html.escape(username)} не найден.</b>", parse_mode='HTML')
            return BROADCAST_TARGET
    elif target_str.isdigit():
        target_id = int(target_str)
        if not db.get_user(target_id):
            await update.message.reply_text(f"<b>❌ Пользователь с ID {target_id} не найден.</b>", parse_mode='HTML')
            return BROADCAST_TARGET
    else:
        await update.message.reply_text("<b>❌ Неверный формат. Введите @username или ID.</b>", parse_mode='HTML')
        return BROADCAST_TARGET
    context.user_data['broadcast_target_id'] = target_id
    await update.message.reply_text("<b>📝 Введите сообщение для отправки:</b>", parse_mode='HTML')
    return BROADCAST_MESSAGE

async def broadcast_message_input(update: Update, context):
    ...
    if target == "broadcast_all":
        ...
        for user in users:
            ...
            try:
                ...
                continue
        await update.message.reply_text(f"<b>✅ Рассылка завершена. Отправлено {count} пользователям.</b>", parse_mode='HTML')
    else:
        uid = context.user_data['broadcast_target_id']
        try:
            await context.bot.send_message(chat_id=uid, text=text, parse_mode='HTML')
            await update.message.reply_text("<b>✅ Сообщение отправлено.</b>", parse_mode='HTML')
        except Exception as e:
            await update.message.reply_text(f"<b>❌ Не удалось отправить: {e}</b>", parse_mode='HTML')
    return ConversationHandler.END

# ---------- Диалог блокировки ----------
async def ban_target_input(update: Update, context):
    target_str = update.message.text.strip()
    target_id = None
    if target_str.startswith('@'):
        username = target_str[1:]
    ...
    await update.message.reply_text(f"<b>🚫 Пользователь {target_id} заблокирован.</b>", parse_mode='HTML')
    return ConversationHandler.END
...
def main():
    ...
    broadcast_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(broadcast_type_choice, pattern="^broadcast_all$|^broadcast_one$|^cancel_broadcast$")],
        states={
            BROADCAST_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_target_input)],
            BROADCAST_MESSAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_message_input)],
        },
    ...
    ban_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.TEXT & ~filters.COMMAND, ban_target_input)],
        states={
            BAN_INPUT_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, ban_target_input)],
        },
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
    )
