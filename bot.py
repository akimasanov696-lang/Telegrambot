...
async def log_referral_event(context, referrer_id, new_user_id, new_user_name, event_type, details=""):
    """Отправляет лог в топик JOIN о переходе по реферальной ссылке."""
...
async def handle_referral(context, ref_id: int, new_user_id: int, new_user_name: str, is_new_user: bool):
    """
    Обрабатывает реферальную ссылку и отправляет уведомления.
...
async def start_registration(update: Update, context):
    """Точка входа в регистрацию для новых пользователей."""
    ...
    if context.args and context.args[0].startswith("check_"):
        ...
        if check and check['is_active'] and check['current_activations'] < check['max_activations']:
            # Если пользователь уже существует, активируем чек
            if db.get_user(user_id):
                if check['password']:
                    ...
                    # Завершаем регистрацию, чек обрабатывается отдельно
                ...
                # Новый пользователь с чеком – просим сначала зарегистрироваться
                await update.message.reply_text("<b>🔗 Сначала пройдите регистрацию, затем повторите активацию чека.</b>", parse_mode='HTML')
                # Продолжим регистрацию без чека, код сохранён не будет
                # Очищаем args, чтобы не мешало
                context.args = []
        else:
            if db.get_user(user_id):
    ...
    # Если пользователь уже зарегистрирован, просто показываем меню
    if db.get_user(user_id):
        await show_main_menu(update, context)
        return ConversationHandler.END

    # Сохраняем ref_id, если есть
    ref_id = None
    if context.args:
...
async def check_sub_callback(update: Update, context):
    ...
    if await check_subscription(user_id, context):
        # Подписался – переходим к капче
        # Удаляем предыдущее сообщение, чтобы не мусорить
        try:
...
async def send_captcha(update: Update, context):
    """Отправляет капчу и переводит в состояние CAPTCHA."""
    user_id = update.effective_user.id
    num1, num2 = random.randint(1, 10), random.randint(1, 10)
    captcha_storage[user_id] = num1 + num2
    # Для callback используем message.reply_text
...
async def captcha_handler(update: Update, context):
    ...
    # Капча пройдена. Проверяем подписку ещё раз (вдруг отписался)
...
async def button_handler(update: Update, context):
    ...
    if data == "profile":
        ...
        text = (
            ...
            f"Сейчас у тебя: {ref_count} друзей"
        )
        if need > 0:
            text += f" (нужно ещё {need})"
        text += f"\n\n🔗 Твоя личная ссылка:\n{html.escape(ref_link)}\n\n"
        ...
        user_data = db.get_user(user_id)
        ref_count = db.get_referrals_count(user_id)
        need_refs = max(0, 5 - ref_count)
        text = f"<b>💰 Баланс: {user_data['stars']} ⭐\n\n🎁 Выберите подарок:</b>"
        if need_refs > 0:
            text += f"\n<b>❗ Для вывода нужно привести минимум 5 друзей (у тебя {ref_count}, осталось {need_refs})</b>"
        keyboard = []
        row = []
        for gift in gifts:
        ...
        if ref_count < 5:
            ...
            await edit_or_reply(query,
                ...
                f"У тебя: {ref_count} друзей, пригласи ещё {need}.</b>",
